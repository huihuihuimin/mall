...
public interface MessageDao
{
    ...
    @Select({"select ", SELECT_FIELDS, " from ", TABLE_NAME, " where conversation_id=#{conversationId} order by id " +
    ...
    @Select({"select ", INSERT_FIELDS, " ,count(id) as id from ( select * from ", TABLE_NAME, " where " +
            "from_id=#{userId} or to_id=#{userId} order by id desc) tt group by conversation_id  order by " +
    ...
    void initUnreadCount(String conversationId);
}
